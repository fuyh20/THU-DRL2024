...
class SACAgent(TD3Agent):
    def __init__(self, state_size, action_size, action_space, hidden_dim, lr_actor, lr_critic, lr_alpha, gamma, tau, nstep, target_update_interval, log_std_min, log_std_max, device):
        ...
        self.alpha = torch.zeros(1, requires_grad=True, device=device)
        self.alpha_optimizer = torch.optim.AdamW([self.alpha], lr=lr_alpha)
    ...
    def get_Qs(self, 
        ...
        with torch.no_grad():
            action_target, log_prob = self.actor_net.evaluate(next_state, sample=True)
            q1_target = self.critic_target(next_state, action_target)
            q2_target = self.critic_target_2(next_state, action_target)
            Q_target = reward + self.gamma * (1 - done) * (torch.min(q1_target, q2_target) - self.alpha * log_prob)
    ...
    def get_alpha_loss(self, 
        ...
        ############################
        # YOUR IMPLEMENTATION HERE #
        alpha_loss = -(self.alpha * (action_log_prob + self.target_entropy).detach()).mean()
        ############################
        return alpha_loss
